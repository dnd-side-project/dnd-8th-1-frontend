...
const ProfileInfo = ({
  ...
  return (
    <Center className="flex-col">
      ...
      {!!(!isEmptyFiled(location) || genres.length) && (
        <>
          <Spacer size={14} />
          <div className="flex gap-[8px]">
            {!isEmptyFiled(location) && !!location && (
      ...
      {!isEmptyFiled(startDate) && (
        <>
          <Spacer size={15.5} />
          <div className="flex items-center gap-[8.82px]">
            <Icon icon="heart" size={16} />
            <span className="text-body2 text-gray-100">
              춤과 <span className="font-bold">{months}개월 째</span> 함께하는
              중
            </span>
          </div>
        </>
